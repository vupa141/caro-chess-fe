<template>
    <div class="bg-gray-100 rounded-md shadow-lg min-h-[100px] mb-10 flex justify-between px-10 py-3">
        <div class="x-player">
            <img
        ...
        <div class="w-60">
            <div :class="`text-center turn bg-red-500 rounded-lg py-2 px-5 text-white h-fit ${props.isYourTurn ? 'bg-green-500' : 'bg-red-500'}`">
                <div>
            ...
            <div
                :class="
                    `time font-bold text-xl text-center mt-2
                    ${ time >= 40 ? 'text-green-500' : `${time >= 20 ? 'text-yellow-500' : 'text-red-500'}`}
                    `
                "
            > {{ time  }}</div>
        </div>
